"""DCT-based frequency-graded patch selection — vendored from
shilinyan99/AIDE (data/dct.py). Selects the most/second-most "min"/"max"
frequency-graded 32x32 patches from a single [C,H,W] image via a discrete
argsort (non-differentiable selection; the returned pixel values still carry
gradient through the unfold/fold/filter ops)."""
...
def DCT_mat(size):
    m = [[(np.sqrt(1. / size) if i == 0 else np.sqrt(2. / size)) *
          np.cos((j + 0.5) * np.pi * i / size) for j in range(size)] for i in range(size)]
    return m
...
class DCT_base_Rec_Module(nn.Module):
    """x: [C, H, W] -> 4 tensors of [C, window_size, window_size]."""
    ...
    def __init__(self, window_size=32, stride=16, output=256, grade_N=6, level_fliter=[0]):
        ...
        self.grade_filters = nn.ModuleList([
            Filter(window_size, window_size * 2. / grade_N * i, window_size * 2. / grade_N * (i + 1), norm=True)
    ...
    def forward(self, x):
        ...
        maxmax_idx1 = max_idx[0] if len(max_idx) == 1 else max_idx[1]

        min_idx = idx[:N]
        minmin_idx = idx[0]
        minmin_idx1 = idx[0] if len(min_idx) == 1 else idx[1]

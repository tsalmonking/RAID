"""AIDE model — vendored from shilinyan99/AIDE (models/AIDE.py). Fuses
low-level DCT/SRM patch features (dual ResNet-50-on-HPF branches over the
4 selected patches) with high-level OpenCLIP ConvNeXt-XXLarge semantics
(over the full image) through a 2-layer MLP head.
"""
...
class HPF(nn.Module):
    def __init__(self):
        ...
        hpf_weight = nn.Parameter(hpf_weight.repeat(1, 3, 1, 1), requires_grad=False)
...
class ResNet(nn.Module):
    """SRM/HPF-branch ResNet-50: takes the 30-channel HPF output, returns a
    pooled 2048-d feature (its own .fc is unused by AIDE_Model.forward)."""
...
class AIDE_Model(nn.Module):
    def __init__(self, resnet_path=None, convnext_path=None):
        super().__init__()
        self.hpf = HPF()
    ...
    def forward(self, x):
        b, t, c, h, w = x.shape
        x_minmin = self.hpf(x[:, 0])
        x_maxmax = self.hpf(x[:, 1])
        x_minmin1 = self.hpf(x[:, 2])
        x_maxmax1 = self.hpf(x[:, 3])
        tokens = x[:, 4]
